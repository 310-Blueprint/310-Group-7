...
This project uses **Tailwind CSS v4**, which reads custom colors from an `@theme` block instead of a `tailwind.config.js` file. Since v4 has no JS-based `presets` option, the color scheme is defined as a standalone preset file at [`src/styles/preset.css`](src/styles/preset.css) and imported into [`src/index.css`](src/index.css):
...
Each `--color-*` variable in the preset's `@theme` block automatically generates the matching Tailwind utility classes (no extra config needed). Use them directly in JSX:
...
- To add or change a color, edit the `@theme` block in `src/styles/preset.css`; Tailwind picks up the new utilities automatically on save.
